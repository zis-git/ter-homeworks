...
locals.tf — общие ресурсы ВМ и метаданные (без дублирования)

""
locals {
  # Ресурсы для ВМ storage (можно переиспользовать и для других ВМ)
  vm_resources_storage = {
    cores         = 2
    memory        = 2
    core_fraction = 20
  }

  # Единый metadata-блок: включен serial-port и подставляется SSH-ключ
  metadata_common = {
    serial-port-enable = 1
    ssh-keys           = "ubuntu:${chomp(file("~/.ssh/id_ed25519.pub"))}"
  }
}
""
что SSH-ключ не захардкожен, а читается функцией
